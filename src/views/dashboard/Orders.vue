...
export default {
  ...
  methods: {
    getOrders(pages = 1) {
      this.isLoading = true;
      const api = `${this.apiPath}${this.uuid}/admin/ec/orders?page=${pages}`;
      this.$http.get(api)
        .then((res) => {
    ...
    setOrderPaid(item) {
      let api = `${this.apiPath}${this.uuid}/admin/ec/orders/${item.id}/paid`;
      if (!item.paid) {
        api = `${this.apiPath}${this.uuid}/admin/ec/orders/${item.id}/unpaid`;
      }
